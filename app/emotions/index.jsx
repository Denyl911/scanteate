...
export default function Emotions() {
  ...
  // Estado para controlar si la pantalla está enfocada
  ...
  // Implementación de useFocusEffect para manejar el estado de foco
  useFocusEffect(
    useCallback(() => {
      getUser();
      ...
      return () => {
        ...
        setIsFocused(false);
        // Opcional: Limpiar el estado de la foto al salir para que no se muestre al volver
        setFotoUri(null);
        setEmotion("Escaner de Emociones");
        setScanText("ESCANEAR");
        setColor("text-sky-900");
        setBorder("border-sky-900");
        // Asegúrate de detener cualquier proceso de cámara si estuviera activo
        if (cameraRef && cameraRef.stopRecording) {
          cameraRef.stopRecording(); // Si estuvieras grabando un video
        }
      };
    }, [])
  );
  ...
  async function scanFace() {
    ...
    try {
      // Asegúrate de que cameraRef esté disponible antes de tomar la foto
      if (!cameraRef) {
  ...
  return (
    <View className="h-[100%]">
      ...
      <View className="flex flex-row justify-between items-center p-2 bg-white">
        ...
        <Image
          source={require("../../assets/images/SNT+B.png")}
      ...
      <View className="flex- items-center">
        {/* Renderiza CameraView solo si la pantalla está enfocada Y no hay fotoURI */}
        {isFocused && !fotoUri && (
          <View
        ...
        {/* Asegúrate de que ShowImage se muestre solo si hay fotoUri */}
        {fotoUri && <ShowImage show={true} />}
      </View>
      <View>
